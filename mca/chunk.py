...
class Chunk:
    ...
    def get_section(self, y: int) -> nbt.TAG_Compound:
        ...
        """
        if y < -4 or y > 19:
            raise ValueError(f'Y ({y!r}) must be in range of -4 to 19')
    ...
    def get_block(self, x: int, y: int, z: int, section: Union[int, nbt.TAG_Compound]=None) -> Block:
        ...
        """
        if x < 0 or x > 15:
            raise ValueError(f'X ({x!r}) must be in range of 0 to 15')
        if z < 0 or z > 15:
            raise ValueError(f'Z ({z!r}) must be in range of 0 to 15')
        if y < -64 or y > 319:
            raise ValueError(f'Y ({y!r}) must be in range of -64 to 319')
    ...
    def stream_blocks(self, index: int=0, section: Union[int, nbt.TAG_Compound]=None) -> Generator[Block, None, None]:
        ...
        if isinstance(section, int) and (section < -4 or section > 19):
            raise ValueError(f'section ({section!r}) must be in range of -4 to 19')
        ...
        states = section['block_states'][0]
        palette = section['block_states'][1]
